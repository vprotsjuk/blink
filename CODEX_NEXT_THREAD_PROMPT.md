...
Вопросы, которые нельзя потерять после компакта: `Paste Attachment` принимает обычные file URLs из буфера (PDF, Excel, изображения и другие файлы), а `Paste Screenshot` принимает изображение только когда file URLs отсутствуют и делает timestamped JPEG; папки не копируются рекурсивно. Drag-and-drop на строки пока не реализован отдельным путём: если добавлять, принимать только обычные файлы, использовать существующую draft-папку, подсвечивать строку, отклонять каталоги и запрещать drop в замороженную History.

Активные/Upcoming события всегда могут открыть или создать пустую папку вложений; History только открывает существующую. Поиск включает имена и расширения локальных вложений. JSON и папки остаются источником истины; SQLite допустим позже только как восстанавливаемый индекс при доказанной необходимости масштабирования.
