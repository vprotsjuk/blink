...
- [x] Step 1: Write failing tests for non-recurring owner IDs, recurring `series_id` owners, malformed/absent manifests, and duplicate events that preserve content but receive a new ID and clean lifecycle state.
- [x] Step 2: Run `.venv/bin/python -m unittest test_agenda_store -q` and confirm the new tests fail.
- [x] Step 3: Implement pure path/manifest helpers; reject path traversal and never store absolute paths in event metadata.
- [x] Step 4: Add duplicate construction without changing the source record; preserve recurrence only when explicitly requested and keep new events unfinished/enabled.
- [x] Step 5: Run the targeted tests and `python -m py_compile app/attachment_store.py app/agenda_store.py`.
- [x] Step 6: Commit `feat: define local attachment metadata and duplicate events`.
...
- [x] Step 1: Add failing tests proving old events without manifests remain valid, malformed manifests do not crash the watcher, and personal notifications add exactly one `📎` when files exist.
- [x] Step 2: Add tests for multiline title/description projection: local values remain untouched and the push projection is bounded by UTF-8 bytes without splitting invalid sequences.
- [x] Step 3: Implement normalized `attachments`/`has_attachments` fields in watcher validation while keeping the watcher blind to attachment bytes.
- [x] Step 4: Implement a single shared personal push projection for compact title and byte-safe body; leave Weather/Astronomy formatters unchanged.
- [x] Step 5: Run `.venv/bin/python -m unittest test_watcher test_notification_format -q` and the full Python suite.
- [x] Step 6: Commit `feat: add attachment marker and safe text projection to pushes`.
...
- [x] Step 1: Add failing Swift smoke cases for draft creation, multi-file copy, JPEG screenshot naming, save finalization, cancel cleanup, and old agenda records without attachment fields.
- [x] Step 2: Implement `event_data/` path resolution relative to the existing Blink root; do not introduce Application Support or a second data root.
- [x] Step 3: Implement draft markers and stale-draft cleanup limited to Blink-created draft directories.
- [x] Step 4: Add atomic finalization and rollback/error reporting so a failed Save does not create an event claiming files it does not own.
- [x] Step 5: Add immutable History rules: History rows cannot open Edit; Duplicate creates a fresh ID, fresh owner folder, and leaves the source unchanged; Delete confirms and sends attachments to Trash.
- [x] Step 6: Run `cd app/BlinkSwiftUI && swift run BlinkSwiftUITestRunner` and `swift build -c release`.
- [x] Step 7: Commit `feat: add Blink-local attachment workspace and frozen history`.
...
- [x] Step 8: Commit `feat: add multiline editor and event context actions`.
...
- [x] Step 1: Run `.venv/bin/python -m unittest -q` and record the exact count (128).
- [x] Step 2: Run Python compilation checks for watcher and attachment/agenda/notification modules.
- [x] Step 3: Run the available `plutil`/bundle checks; the installed Info.plist remains valid.
- [x] Step 4: Build and install the Swift release executable inside `Blink.app/Contents/MacOS/Blink`; restart Blink and verify the watcher after the build succeeds.
- [x] Step 5: Run `./status_watcher.command` and verify a fresh heartbeat; Swift/Python contract tests cover files, screenshot naming, multiline text, History duplicate/freeze, and `📎` projection.
- [x] Step 6: Verify `git status` contains no private runtime files, attachment bytes, draft bytes, or build artifacts.
- [x] Step 7: Re-read the design and plan checklists line by line; all implementation requirements are complete.
- [x] Step 8: Commit only the intended source/docs changes with `feat: add local event attachments and frozen history`.
...
- [x] Weather, Astronomy, ntfy scheduling, lamp, and existing lifecycle behavior remain unchanged.
- [x] All Blink runtime files remain under the Blink root and public GitHub remains clean.
