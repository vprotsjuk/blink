...
The approved Blink-local event attachments feature is implemented. It adds multiline event text, draft/permanent attachment folders under `event_data/`, JPEG screenshot paste, paperclip-only personal pushes, frozen History, double-click editing in Today/Upcoming, compact row attachment filename/type lists, folder buttons, and tab-aware context actions. The Today header now owns a round blue `+` action for New Event, and every navigation tab has a subtle pointer-hover state. The design and implementation plan remain in `docs/superpowers/specs/2026-09-09-event-attachments-and-history-freeze-design.md` and `docs/superpowers/plans/2026-09-09-event-attachments-and-history-freeze.md`.
...
- Clipboard and drag/drop policy is intentionally small: file URLs use `Paste Attachment`, image-only clipboard data uses `Paste Screenshot`, and folders use `Attach Files`; drag/drop is not an attachment path.
...
- If the clipboard contains an Excel/PDF file URL, should `Paste Screenshot` import it? **Implemented:** no; show `Paste Attachment` instead and use the existing file-import path.
...
- Should drag-and-drop be added to event rows? **Recommended future shortcut:** accept one or more regular file URLs, stage them in the open event draft, highlight the row while hovering, and reject directories with an explanatory message. Do not add a second storage or scheduler path.
...
2. For future changes, keep `event_data/` private and rerun the full release gate after code changes.
3. If a future UX pass is requested, inspect the three event tabs again at normal and reduced window sizes.
