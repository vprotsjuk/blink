...
```json
{
  "version": 1,
  "type": "DONE",
  "command_id": "20260912154532-482193775",
  "event_id": "event-...",
...
`command_id` is a strict transport identity generated by the Shortcut. The
preferred Apple-Shortcuts-native form is `^[0-9]{14}-[0-9]{9}$`, for example
`20260912154532-482193775`; UUIDv4 remains accepted for backward compatibility.
It is not the event identity. `event_id` is the exact production occurrence ID shown
...
event.id
  ...
  -> Shortcut generates a fresh native transport command_id
...
than a serialized agenda record. The Shortcut generates the complete native
transport ID locally; it must never reuse the event ID as transport identity.
...
formatter so they cannot drift. The Mac-side ntfy action is implemented behind
`BLINK_NTFY_DONE_ACTION_ENABLED` (off by default); exact iOS
`shortcuts://` acceptance remains a manual Phase 4B check.
...
`transfer_id` is a strict transport identity: the preferred form is
`^[0-9]{14}-[0-9]{9}$` (`yyyyMMddHHmmss` local timestamp plus a 9-digit random
number), while UUIDv4 remains accepted for backward compatibility. The Mac allocates the production
...
| `transfer_id` | no event field; processed ledger key | Combined timestamp-random transport dedupe key (UUIDv4 accepted for compatibility). | Never use timestamp alone or transfer ID as event ID. |
...
The importer recognizes only strict native/UUID transport-ID names and exact matching stems.
...
Use the preferred `yyyyMMddHHmmss-9digit` transport ID for DONE `command_id`
and CREATE `transfer_id`; UUIDv4 remains accepted for backward compatibility.
The whole combined ID is the identity. Never use the filename, original display
name, or timestamp alone as business identity.
...
Filename validation is strict: native/UUID transport stem, exact
`.event.json`/`.done.json` and `.ready` suffixes, and an optional attachment
whose basename contains the same stem. JSON must not contain absolute paths, `..`, slashes, NULs, shell syntax,
...
contains the transport ID and result, and `mailbox_runtime.json` records the
...
3. DONE uses the preferred `yyyyMMddHHmmss-9digit` `command_id`; CREATE uses
   the same format for `transfer_id`. UUIDv4 remains accepted for backward
   compatibility. The whole combined string is transport identity only.
...
6. `blinker_intent.minutes_before` is the transport form; no absolute
   ...
   attachment limit remains an open owner value. Transport-ID tombstones are retained
...
```text
iPhone Shortcut
    | write <transport_id>.done.json
    | write <transport_id>.ready last
    v
...
```text
iPhone Shortcut
    | write <transport_id>.event.json
    | write optional <transport_id>.attachment.ext
    | write <transport_id>.ready last
    v
private iCloud Blink_Production/ToMac
    | file becomes locally available
...
| Ledger retention | Deleting old IDs can permit replay after event/provenance deletion. | Keep transport-ID tombstones indefinitely by default; size estimates are operationally small. |
...
- native `yyyyMMddHHmmss-9digit` transport IDs (with UUIDv4 compatibility) and Mac-generated production event IDs;
